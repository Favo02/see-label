...
>>>>>>> origin/feat/toast
    ...
    async function apiCall() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                toast.error('Error sending file', toastOptions);
            }
        } catch (e) {
            toast.error('Error sending file!', toastOptions);
            console.log(e)
    ...
    const promptAsync = useCallback(() => {
        return new Promise((resolve) => {
            setPopupCallback(() => resolve);
            openPopup();
        });
    }, []);

    const handleLabelSubmit = (newLabel) => {
        if (popupCallback) {
            popupCallback(newLabel);
        }
        closePopup();
    };
            ...
            <div className="mx-auto max-w-[500px]">
                ...
                <div className="flex justify-center items-center">
                    <label className="inline-flex items-center justify-center cursor-pointer w-full">
                        <input type="checkbox" value={normalizeSize} className="sr-only peer" defaultChecked
                               onChange={e => setNormalizeSize(e.target.checked)}/>
                        <div
                            className="relative w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-sky-300 dark:peer-focus:ring-sky-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-sky-600"/>
                        <span
                            className="ms-3 text-lg font-bold text-white dark:text-gray-300">Normalize image size</span>
                    </label>
                </div>
        ...
        return (
            <div className="mx-auto">
                ...
                <div className="mx-auto flex flex-row justify-center align-center items-center gap-2">
                    ...
                    <button disabled
                            className="text-white text-lg font-bold bg-gray-700 hover:bg-gray-700 rounded-lg text-sm px-5 py-2.5">Import
                        analyzed objects
                    </button>
                ...
                <div className="mx-auto flex flex-row justify-center align-center items-center gap-2">
                    ...
                    <div
                        ...
                        <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                               htmlFor="file_input">Import analyzed objects</label>
    ...
    return (
        <div>
            ...
            <h1 className="text-white text-3xl font-bold text-center pt-6 pb-4">Click on the image to manually add
                polygons or Apply some filters</h1>

            <div className="mx-auto flex flex-row justify-center align-center items-center gap-2">

                <button onClick={() => exportImageObjects(false)}
                        className="text-white text-lg font-bold bg-sky-700 hover:bg-sky-800 rounded-lg text-sm px-5 py-2.5">Export
                    Objects only (JSON)
                </button>

                <button onClick={() => exportImageObjects(true)}
                        className="text-white text-lg font-bold bg-sky-700 hover:bg-sky-800 rounded-lg text-sm px-5 py-2.5">Export
                    Image + Objects (JSON)
                </button>
            ...
            <h1 className="text-white text-md font-bold italic text-center pt-2">NOTE: Only objects NOT filtered out
                will be exported</h1>

            <div className="w-full flex flex-row mt-4">

                <div className="w-2/3">
                    <ImageDrawer refresh={refresh} objects={filtered} image={image.url} size={imageSize}
                                 onNewPolygon={newDrawnPolygon}/>
                </div>
                ...
                <div className="w-1/3">
                    ...
                    <div>
                        <div className="text-center">
                            ...
                            <input type="range" min={0.25} max={1} value={confidence} step="0.01"
                                   onChange={(e) => updateConfidence(e.target.value)}
                                   className="w-full max-w-[200px] h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700"/>
                        </div>
                    </div>
                    ...
                    {objects.reduce((uniqueObjects, obj) => {
                        ...
                        <h2 key={obj.object_name} onClick={() => updateFilters(obj.object_name)}
                            className={`cursor-pointer text-2xl italic text-white text-center ${filtered.find(obj2 => obj2.object_name === obj.object_name) ? "" : "line-through"}`}>
